...
{
    public partial class ReceiveTC : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    txtwrkId.Text = Genaral.UrlDecrypt(HttpUtility.UrlDecode(Request.QueryString["ReferID"]));
                }
        ...
        public void LoadWODetails()
        {
            try
            {
                ...
                if(txtActiontype.Text == "V")
                {
                    objTc.LoadWODetails(objTc,"View"); // To View Already Approved Record
                }
                else
                {
                ...
                if (objTc.sNewDTRCode != null)
                {
                    txtTCCode.Text = objTc.sNewDTRCode;
                }
                else
                {
                    txtTCCode.Text = objTc.sOldDTRCode;
                }
        ...
        public void LoadSearchWindow()
        {
            try
            {
                ...
                strQry += " AND \"TC_MAKE_ID\" = \"TM_ID\" AND CAST(\"TC_LOCATION_ID\" AS TEXT) like '" + clsStoreOffice.GetStoreID(objSession.sStoreID) + "' AND ";
                strQry += " \"EST_REPAIRER\" = '" + cmbRepairer.SelectedValue + "' and \"TC_CAPACITY\"='" + txtCapacity.Text+ "'  AND {0} like %{1}% &";
                strQry += "DBColName=CAST(\"TC_CODE\" AS TEXT)~\"TM_NAME\"~CAST(\"TC_CAPACITY\" AS TEXT)&";
            ...
            try
            {
                ...
                if (ValidateForm() == true)
                {
                    ...
                    if (txtActiontype.Text == "A" || txtActiontype.Text == "R" || txtActiontype.Text == "D" || txtActiontype.Text == "M")
                    {
                        ...
                        {
                            if(txtOldTcCode.Text == txtTCCode.Text)
                            {
                                hdfTCType.Value = "1";
                            }
                            else
                    ...
                    if (hdfWFDataId.Value == "0")
                    {
                        objMinor.sFormName = "ReceiveMinorTC";
                    }
                    if (hdfTCType.Value == "2")
                    {
                        objMinor.sFormName = "ReceiveMinorTC";
                    }
                    //else if (hdfWFDataId.Value != "0" && hdfTCType.Value != "2")
                    //{
                    //    objMinor.sFormName = "";
                    //}
                        //Approve                    

                        WorkFlowObjects(objMinor);
                    ...
                    if (txtActiontype.Text == "M")
                    {
                        ...
                        objMinor.sActionType = txtActiontype.Text;
                        objMinor.sCrBy = hdfCrBy.Value;

                        Arr = objMinor.SaveDetails(objMinor);

                        if (objSession.sTransactionLog == "1")
                        {
                            Genaral.TransactionLog(objSession.sClientIP, objSession.UserId, " (ReceiveMinorTC) Failure ");
                        }

                        if (Arr[1].ToString() == "0")
                        {
                            hdfWFDataId.Value = objMinor.sWFDataId;
                            ApproveRejectAction();
        ...
        public void ApproveRejectAction()
        {
            try
            {
                ...
                objApproval.sFailType = hdfTCType.Value;
                if (objApproval.sFailType == "2")
                {
        ...
        protected void GetDataFromXML(string sWFO_ID)
        {
            try
            {
                ...
                if(objReceive.sNewDTRCode != null && objReceive.sNewDTRSLNo != null)
                {
                    txtTCCode.Text = objReceive.sNewDTRCode;
                    txtCapacity.Text = objReceive.sNewDTRCapacity;
                    txtSLNo.Text = objReceive.sNewDTRSLNo;
        ...
        protected void cmdClose_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Response.Redirect("RecieveMinorTC.aspx", false);
                }
