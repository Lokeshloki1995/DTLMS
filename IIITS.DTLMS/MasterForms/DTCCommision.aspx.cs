    ...
    public partial class DTCCommision : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                cmdloctype.Attributes.Add("readonly", "readonly");
        ...
        public void LoadDtcDetails(string strId)
        {
            try
            {
                ...
                #region  added on 21-11-2023 by santhosh
                ddldtcmeters.SelectedValue = objDtcMaster.sDTCMeters; 
                if (ddldtcmeters.SelectedValue == "1")
                {
                    DivKWHReading.Visible = true;
                    txtKWHReading.Visible = true;
                    txtKWHReading.Enabled = true;
                }
                else
                {
                    DivKWHReading.Visible = false;
                    txtKWHReading.Visible = false;
                    txtKWHReading.Enabled = false;
                }
                #endregion


                cmdloctype.Text = objDtcMaster.locationtype;
                ...
                {
                    objDTC.GetImagePathForNew(objDTC);
                }
                else
                {
        ...
        protected void ddldtcmeters_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (ddldtcmeters.SelectedValue == "1")
        ...
        protected void cmdSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (ValidateForm() == true)
                {
                    ...
                    objDtcCommision.locationtype = cmdloctype.SelectedValue;
                    //objDtcCommision.sFeederCode = cmbFeeder.SelectedValue;
                    objDtcCommision.sFeederCode = cmbFeeder.SelectedItem.Text.Substring(0, 6);
                    ...
                    #region // addded on 21-11-2023 by santhosh
        ...
        bool ValidateForm()
        {
            ...
            try
            {
                ...
                {
                    if (!System.Text.RegularExpressions.Regex.IsMatch(txtKWHReading.Text, "^(\\d{1,7})?(\\.\\d{1,3})?$"))
                    {
                        ShowMsgBox("Enter Valid KWH Reading (eg:1234567.111)");
                        return false;
                    }

                    if (!System.Text.RegularExpressions.Regex.IsMatch(txtKWHReading.Text, "[-+]?[0-9]{0,7}\\.?[0-9]{0,3}"))
                    {
                        ShowMsgBox("Enter Valid KWH Reading (eg:1234567.111)");
                        return false;
                    }
        ...
        protected void cmdReset_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ddldtcmeters.SelectedIndex = 0; // added by santhosh
                ...
                txtTimsCode.Text = string.Empty;
                cmdloctype.SelectedIndex = 0;
                cmbprojecttype.SelectedIndex = 0;
                txtEleDate.Text = string.Empty;
                txtMla.Text = string.Empty;
                txtMP.Text = string.Empty;
                txtEleRateNo.Text = string.Empty;
                txtAvgLoad.Text = string.Empty;
                txtPeakLoad.Text = string.Empty;
                txtSurplusCap.Text = string.Empty;
            }
            catch (Exception ex)
            {
                lblMessage.Text = clsException.ErrorMsg();
